...
const ProjectCard = ({ image, title, description, websiteUrl, readMoreUrl }) => (
  <div className="max-w-3xl grid grid-rows-2 md:grid-rows-none md:grid-cols-2 bg-[#FFFFFF08] border-dashed border m-3 p-3 rounded-lg border-neutral-400">
    <Image width={1000} height={1000} src={image} alt={title} className="w-full border-2 border-neutral-300 rounded-lg" />
      ...
      <div className="w-full flex gap-3 text-xs md:text-sm items-center">
        <a href={websiteUrl} className="group relative inline-flex h-12 items-center justify-center rounded-full bg-neutral-800 px-3 md:px-6 font-medium text-neutral-200">
          ...
          <div className="relative ml-1 h-5 w-5 overflow-hidden">
            <ArrowUpRight className="absolute transition-all duration-200 group-hover:rotate-45" />
          </div>
        </a>
...
const Projects = () => (
  <div className="w-full px-1 md:px-10 flex flex-col gap-10">
